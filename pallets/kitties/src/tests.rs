...
fn create_a_kitty_works_when_have_enough_balance() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(
            KittiesModule::create_kitty(Origin::signed(2)),
        );
    });
}
...
fn breed_a_kitty_works_when_balance_enough() {
    new_test_ext().execute_with(|| {
        assert_ok!(
            KittiesModule::create_kitty(Origin::signed(8)),
        );
        assert_ok!(
            KittiesModule::create_kitty(Origin::signed(8)),
        );
        ...
        assert_ok!(
            KittiesModule::breed(Origin::signed(8), 1, 2),
        );
...
fn transfer_a_kitty_works() {
    new_test_ext().execute_with(|| {
        assert_ok!(
            KittiesModule::create_kitty(Origin::signed(3)),
        );
...
fn sell_a_kitty_fails_when_not_owner() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(KittiesModule::sell_kitty(Origin::signed(1), 1, Some(20)), Error::<Test>::NotOwner);
    });
}
...
fn sell_a_kitty_fails_when_kitty_index_invalid() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(KittiesModule::sell_kitty(Origin::signed(3), 121, Some(20)), Error::<Test>::InvalidKittyIndex);
    });
}
...
fn buy_a_kitty_fails_when_buyer_is_kitty_owner() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(KittiesModule::sell_kitty(Origin::signed(3), 1, Some(9)));
        assert_noop!(KittiesModule::buy_kitty(Origin::signed(3), 1), Error::<Test>::BuyerIsKittyOwner);
    });
}
...
fn buy_a_kitty_fails_when_not_enough_balance() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(KittiesModule::buy_kitty(Origin::signed(1), 1), Error::<Test>::NotEnoughBalance);
    });
}
...
fn buy_a_kitty_fails_when_not_for_sell() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(KittiesModule::buy_kitty(Origin::signed(1), 1), Error::<Test>::KittyNotForSale);
    });
}
...
fn buy_a_kitty_fails_when_kitty_not_exist() {
    new_test_ext().execute_with(|| {
       assert_noop!(KittiesModule::buy_kitty(Origin::signed(1), 1), Error::<Test>::InvalidKittyIndex);
    });
}
